...
static int csi_probe(struct platform_device *pdev)
{
  ...
  /*get irq resource*/
	...
	ret = script_parser_fetch("csi0_para","csi_twi_id", &twi_id , sizeof(int));
	if (ret) {
	...
	{
		ret = script_parser_fetch("csi0_para","csi_twi_addr", &i2c_addr , sizeof(int));
		if (ret) {
			csi_err("fetch csi_twi_addr from sys_config failed\n");
			return -1;
		}
		
		ret = script_parser_fetch("csi0_para","csi_mname", (int *)&ccm , I2C_NAME_SIZE*sizeof(char));
		if (ret) {
	...
	ret = script_parser_fetch("csi0_para","csi_iovdd", (int *)&iovdd , 32*sizeof(char));
	if (ret) {
	...
	ret = script_parser_fetch("csi0_para","csi_avdd", (int *)&avdd , 32*sizeof(char));
	if (ret) {
	...
	ret = script_parser_fetch("csi0_para","csi_dvdd", (int *)&dvdd , 32*sizeof(char));
	if (ret) {
