...
#define sw_hcd_DRIVER_NAME    "sw_hcd_host1"
...
static __s32 pin_init(sw_hcd_io_t *sw_hcd_io)
{
	...
	/* request gpio */
	ret = script_parser_fetch("usbc1", "usb_drv_vbus_gpio", (int *)&sw_hcd_io->drv_vbus_gpio_set, 64);
	if(ret != 0){
...
static __s32 sw_hcd_io_init(__u32 usbc_no, struct platform_device *pdev, sw_hcd_io_t *sw_hcd_io)
{
    ...
    /* open usb lock */
	sw_hcd_io->sie_clk = clk_get(NULL, "ahb_usb1");
...
*******************************************************************************
*/
static irqreturn_t hcd1_generic_interrupt(int irq, void *__hci)
{
	DMSG_DBG_HCD("irq: %s\n", sw_hcd_driver_name);
...
static int __init sw_hcd_init_controller(struct device *dev, int nIrq, void __iomem *ctrl)
{
	 ...
	 */
	sw_hcd->isr = hcd1_generic_interrupt;
...
static int __init sw_hcd_probe(struct platform_device *pdev)
{
	...
	int             irq         = SW_INTC_IRQNO_USB1; //platform_get_irq(pdev, 0);
	__s32 			ret 		= 0;
