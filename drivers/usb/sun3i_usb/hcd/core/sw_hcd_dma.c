...
void sw_hcd_dma_set_config(struct sw_hcd_qh *qh, __u32 buff_addr, __u32 len)
{
	...
	if(qh->hw_ep->sw_hcd->usbc_no == 0){
		usbc_base = USBC0_BASE;
	}else if(qh->hw_ep->sw_hcd->usbc_no == 1){
		usbc_base = USBC1_BASE;
	}else{
		usbc_base = USBC2_BASE;
	}
	...
	if(is_in){ /* ep in, rx*/
		if(qh->hw_ep->sw_hcd->usbc_no == 0){
			...
			usbc_no = DRQ_TYPE_USB1;
		}else{
			usbc_no = DRQ_TYPE_USB2;
		}
...
__s32 sw_hcd_dma_probe(struct sw_hcd *sw_hcd)
{
	...
	}else if(sw_hcd->usbc_no == 1){
		dma_channel = DMACH_DUSB1;
	}else{
		dma_channel = DMACH_DUSB2;
	}
