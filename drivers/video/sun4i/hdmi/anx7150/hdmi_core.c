...
void ANX7150_HW_Interface_Variable_Initial()
{
    ...
    __inf("s_ANX7150_audio_config.i2s_config.audio_channel = %d.\n", s_ANX7150_audio_config.i2s_config.audio_channel);
...
//#ifdef ITU656
    for (i = 0; i < 18; i++)
    {
        ...
        __inf("Video_Timing_Parameter[%x]=%x\n", (unsigned int)i, (unsigned int) ANX7150_video_timing_parameter[i]);
    }
    /*#else
        for(i = 0; i < 18; i++)
        {
    ...
    __inf("\n");
    {
        ...
        switch (ANX7150_video_timing_id)
        {
            ...
            case ANX7150_V720x480i_60Hz_16x9:
                ...
                break;
            default:
                __inf("unknown(video ID is: %x).\n",(unsigned int)ANX7150_video_timing_id);
                break;
        }
    }
    if (c1)//HDMI output
...
uint8 ANX7150_Config_Audio(void)
{
    ...
    __inf("audio_type = 0x%x\n",(unsigned int)s_ANX7150_audio_config.audio_type);
...
{
    __inf("----ANX7150_Set_System_State:%d\n",ss);
...
uint8 ANX7150_Parse_EDID(void)
{
    ...
    __inf("EDIDLength is %d\n",  (unsigned int)ANX7150_edid_length);
        ...
        if (ANX7150_ddc_fifo_full)
        {
            ...
            for (j=0; j<c; j++)
            {
                ...
                if (segment == 0)
                {
                    ...
                    __inf("EDID[0x%x]=0x%x    ", (unsigned int)(length - i + j), (unsigned int) c1);
                }
                else if (segment == 1)
                {
                    ANX7150_EDID_Buf[length - i + j + 0x80] = c1;
                    __inf("EDID[0x%x]=0x%x    ", (unsigned int)(length - i + j + 0x80), (unsigned int) c1);
                }
...
void ANX7150_Parse_VideoSTD()
{
    ...
    while (i < STDReg_tmp)
    {
        ...
        //__inf("ANX7150_edid_result.SVD_ID[%x]=0x%x\n",(unsigned int)i,(unsigned int)ANX7150_edid_result.SVD_ID[i]);
...
uint8 ANX7150_EDID_Checksum(uint8 block_number)
{
    ...
    __inf("edid_block_checksum = 0x%x\n",(unsigned int)edid_block_checksum);
    if ((block_number / 2) * 2 == block_number)
