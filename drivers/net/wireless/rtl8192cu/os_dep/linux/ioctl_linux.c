...
void pre_tx_scan_timer_process (void *FunctionContext)
{
	...
	if ( P2P_STATE_TX_PROVISION_DIS_REQ == pwdinfo->p2p_state )
	{
		...
		{
			_status = rtw_sitesurvey_cmd(adapter, &pwdinfo->tx_prov_disc_info.ssid);
		}
	}
	else if ( P2P_STATE_GONEGO_ING == pwdinfo->p2p_state )
	{
		...
		{
			_status = rtw_sitesurvey_cmd( adapter, NULL );
		}
	}
	else
	{
		DBG_8192C( "[%s] p2p_state is %d\n", __FUNCTION__, pwdinfo->p2p_state );
	}
                               ...
                               union iwreq_data *wrqu, char *extra)
{
	...
	DBG_8192C( "[%s] Role = %d, Status = %d, peer addr = %.2X:%.2X:%.2X:%.2X:%.2X:%.2X\n", __FUNCTION__, pwdinfo->role, pwdinfo->p2p_state,
			pwdinfo->p2p_peer_interface_addr[ 0 ], pwdinfo->p2p_peer_interface_addr[ 1 ], pwdinfo->p2p_peer_interface_addr[ 2 ],
			pwdinfo->p2p_peer_interface_addr[ 3 ], pwdinfo->p2p_peer_interface_addr[ 4 ], pwdinfo->p2p_peer_interface_addr[ 5 ]);
	sprintf( extra, "\n\npeer_ifa=%02x:%02x:%02x%02X:%02X:%02X",
			pwdinfo->p2p_peer_interface_addr[ 0 ], pwdinfo->p2p_peer_interface_addr[ 1 ], pwdinfo->p2p_peer_interface_addr[ 2 ],
			pwdinfo->p2p_peer_interface_addr[ 3 ], pwdinfo->p2p_peer_interface_addr[ 4 ], pwdinfo->p2p_peer_interface_addr[ 5 ]);
...

{
	...
	sprintf( extra, "\n\npeer_deva=%02X:%02X:%02X:%02X:%02X:%02X",
			pwdinfo->rx_prov_disc_info.peerDevAddr[ 0 ], pwdinfo->rx_prov_disc_info.peerDevAddr[ 1 ], 
			pwdinfo->rx_prov_disc_info.peerDevAddr[ 2 ], pwdinfo->rx_prov_disc_info.peerDevAddr[ 3 ],
			pwdinfo->rx_prov_disc_info.peerDevAddr[ 4 ], pwdinfo->rx_prov_disc_info.peerDevAddr[ 5 ]);
	return ret;
                               ...
                               union iwreq_data *wrqu, char *extra)
{
	...
	_rtw_memcpy( peerMACStr , wrqu->data.pointer , 17 );
