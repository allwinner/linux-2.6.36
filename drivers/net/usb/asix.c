...
static void ax88178_status(struct usbnet *dev, struct urb *urb)
{
	...
	struct ax88178_data *ax178dataptr = (struct ax88178_data *)dev->driver_priv;
	int link;
	...
	if (netif_carrier_ok(dev->net) != link) {
		...
		netdev_dbg(dev->net, "ax88178 - Link status is: %d", link);
	}
}
...
static void ax8817x_status(struct usbnet *dev, struct urb *urb)
{
	...
	if (netif_carrier_ok(dev->net) != link) {
		if (link) {
			...
			netif_carrier_off(dev->net);
		netdev_dbg(dev->net, "ax8817x - Link status is: %d", link);
	}
}
...
{
	struct ax88172_int_data *event;
	struct ax88772_data *ax772_data = (struct ax88772_data *)dev->driver_priv;
	int link;
	...
	if (netif_carrier_ok(dev->net) != link) {
		...
		netdev_dbg(dev->net, "ax88772 - Link status is: %d", link);
	}
...
{
	struct ax88172_int_data *event;
	struct ax88772a_data *ax772a_data = (struct ax88772a_data *)dev->driver_priv;
	int link;
	...
	if (netif_carrier_ok(dev->net) != link) {
		...
		netdev_dbg(dev->net, "ax88772a - Link status is: %d", link);
	}
...
{
	struct ax88772b_data *ax772b_data = (struct ax88772b_data *)dev->driver_priv;
	struct ax88172_int_data *event;
	int link;
	...
	if (netif_carrier_ok(dev->net) != link) {
		...
		netdev_dbg(dev->net, "ax88772b - Link status is: %d", link);
	}
...
static void
ax8817x_write_cmd_async(struct usbnet *dev, u8 cmd, u16 value, u16 index, u16 size, void *data)
{
	struct usb_ctrlrequest *req;
	int status;
	struct urb *urb;

	netdev_dbg(dev->net, "ax8817x_write_cmd_async() cmd=0x%02x value=0x%04x index=0x%04x size=%d\n",
		   cmd, value, index, size);
...
static int ax88772b_suspend (struct usb_interface *intf, pm_message_t message)
{
	...
	if (!(*opt & AX_MONITOR_LINK) && !(*opt & AX_MONITOR_MAGIC)) {
		...
		if (ax772b_data->psc & AX_SWRESET_WOLLP) {
			*tmp16 = ax8817x_mdio_read (dev->net,
					dev->mii.phy_id, MII_BMCR);
			ax8817x_mdio_write (dev->net, dev->mii.phy_id,
...
static int ax8817x_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	if ((ret = ax8817x_write_cmd(dev, AX_CMD_WRITE_RX_CTL,
				0x80, 0, 0, NULL)) < 0) {
		netdev_err(dev->net, "send AX_CMD_WRITE_RX_CTL failed: %d", ret);
		goto out2;
	}
	...
	if ((ret = ax8817x_read_cmd(dev, AX_CMD_READ_NODE_ID,
				0, 0, 6, buf)) < 0) {
		netdev_err(dev->net, "read AX_CMD_READ_NODE_ID failed: %d", ret);
		goto out2;
	}
	...
	if ((ret = ax8817x_read_cmd(dev, AX_CMD_READ_PHY_ID,
				0, 0, 2, buf)) < 0) {
		netdev_err(dev->net, "error on read AX_CMD_READ_PHY_ID: %02x", ret);
		goto out2;
	} else if (ret < 2) {
		/* this should always return 2 bytes */
		netdev_err(dev->net, "Read PHYID returned less than 2 bytes: ret=%02x",
				ret);
		ret = -EIO;
		goto out2;
	}
...
static int ax88772_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	if(!buf) {
        ...
        if (!ax772_data) {
                netdev_err(dev->net, "Cannot allocate memory for AX88772 data");
                kfree (buf);
                return -ENOMEM;
        }
        memset (ax772_data, 0, sizeof(*ax772_data));
        dev->driver_priv = ax772_data;
	...
	if ((ret = ax8817x_read_cmd(dev, AX_CMD_READ_PHY_ID,
			0, 0, 2, buf)) < 0) {
		netdev_err(dev->net, "Error reading PHY ID: %02x", ret);
		goto out2;
	} else if (ret < 2) {
		/* this should always return 2 bytes */
		netdev_err(dev->net, "Read PHYID returned less than 2 bytes: ret=%02x",
	...
	if ((ret = ax8817x_write_cmd(dev, AX_CMD_WRITE_RX_CTL,
				0x0000, 0, 0, NULL)) < 0) {
		netdev_err(dev->net, "Failed to reset RX_CTL: %d", ret);
		goto out2;
	}
	...
	if (dev->mii.phy_id == 0x10)
	{
		if ((ret = ax8817x_mdio_read(dev->net,
				dev->mii.phy_id, 2)) != 0x003b) {
			netdev_err(dev->net, "Read PHY register 2 must be 0x3b00: %d",
				ret);
			goto out2;
		}
		...
		if ((ret = ax8817x_write_cmd(dev, AX_CMD_SW_RESET,
					AX_SWRESET_PRL,
					0, 0, NULL)) < 0) {
			netdev_err(dev->net, "Set external PHY reset pin level: %d", ret);
			goto out2;
		}
		msleep(150);
		if ((ret = ax8817x_write_cmd(dev, AX_CMD_SW_RESET,
	...
	if ((ret = ax8817x_write_cmd(dev, AX_CMD_WRITE_MEDIUM_MODE,
		...
		netdev_err(dev->net, "Write medium mode register: %d", ret);
		goto out2;
	}
	...
	if ((ret =
		...
		netdev_err(dev->net, "Failed to set hardware MII: %02x", ret);
		goto out2;
	}
...
static int ax88772a_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	if (!ax772a_data) {
		netdev_err(dev->net, "Cannot allocate memory for AX88772A data");
		kfree (buf);
		return -ENOMEM;
	}
	memset (ax772a_data, 0, sizeof(*ax772a_data));
	dev->driver_priv = ax772a_data;
	...
	if ((ret = ax8817x_read_cmd(dev, AX_CMD_READ_EEPROM, 
		...
		netdev_err(dev->net, "read SROM address 17h failed: %d", ret);
		goto out2;
	}
	...
	if(dev->mii.phy_id != 0x10) {
		netdev_err(dev->net, "Got wrong PHY ID: %02x", dev->mii.phy_id);
		goto out2;
	}
...
static int ax88772b_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	if (!ax772b_data) {
		netdev_err(dev->net, "Cannot allocate memory for AX88772B data");
		kfree (buf);
		return -ENOMEM;
	}
	memset (ax772b_data, 0, sizeof(*ax772b_data));
	dev->driver_priv = ax772b_data;
	...
	if ((ret = ax8817x_read_cmd (dev, AX_CMD_READ_EEPROM,
		...
		netdev_err(dev->net, "read SROM address 18h failed: %d", ret);
		goto err_out;
	}
	le16_to_cpus(tmp16);
	...
	*tmp16 = ax8817x_mdio_read(dev->net, dev->mii.phy_id, 0x12);
	ax8817x_mdio_write(dev->net, dev->mii.phy_id, 0x12,
			((*tmp16 & 0xFF9F) | 0x0040));
	...
	if ((ret = ax8817x_write_cmd (dev, AX_CMD_SW_RESET,
		...
		netdev_err(dev->net, "Failed to configure PHY power saving: %d", ret);
		goto err_out;
	}
...
static void Vitess_8601_Init (struct usbnet *dev, int State)
{
	...
	case 0:	// tx, rx clock skew
		ax8817x_swmii_mdio_write (dev->net, dev->mii.phy_id, 31, 1);
		ax8817x_swmii_mdio_write (dev->net, dev->mii.phy_id, 28, 0);
		ax8817x_swmii_mdio_write (dev->net, dev->mii.phy_id, 31, 0);
		break;

	case 1:
		ax8817x_swmii_mdio_write (dev->net,
		...
		ax8817x_swmii_mdio_write (dev->net,
				dev->mii.phy_id, 18,
				ax8817x_swmii_mdio_read (dev->net,
						dev->mii.phy_id, 18));

		reg = (ax8817x_swmii_mdio_read (dev->net,
...
ax88178_phy_init (struct usbnet *dev, struct ax88178_data *ax178dataptr)
{
	...
	if (!ax178dataptr->UseGpio0) {
		i = 1000;
		while (i--) {
			PhyID1 = ax8817x_swmii_mdio_read (dev->net, 
	...
	ax178dataptr->UseRgmii = 0;
	if (ax178dataptr->PhyMode == PHY_MODE_MARVELL) {
		PhyReg = ax8817x_swmii_mdio_read(dev->net, 
					dev->mii.phy_id, 27);
		if (!(PhyReg & 4)) {
...
//		Vitess_8601_Init (dev, 0);
	...
	if (ax178dataptr->PhyMode != PHY_MODE_ATTANSIC_V0) {
		// software reset
		ax8817x_swmii_mdio_write (
			dev->net, dev->mii.phy_id, GMII_PHY_CONTROL,
			ax8817x_swmii_mdio_read (
				dev->net, dev->mii.phy_id, GMII_PHY_CONTROL)
				| GMII_CONTROL_RESET);
		msleep (1);
	}
	...
	// read phy register 0
	PhyCtrl = ax8817x_swmii_mdio_read (dev->net, 
	...
	if (ax178dataptr->PhyMode == PHY_MODE_VSC8601)
	{
		...
		TempShort = ax8817x_swmii_mdio_read (dev->net, 
...
static int ax88178_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	if (!ax178dataptr) {
		netdev_err(dev->net, "Cannot allocate memory for AX88178 data");
		ret = -ENOMEM;
		goto error_out;
	}
	memset (ax178dataptr, 0, sizeof (struct ax88178_data));
	dev->driver_priv = ax178dataptr;
	...
	if (ax178dataptr->UseGpio0) {
		...
		if (ax178dataptr->PhyMode == PHY_MODE_MARVELL) {
			...
			if ((ret = ax8817x_write_cmd (dev, AX_CMD_WRITE_GPIOS,
					...
					0, 0, NULL)) < 0) {
				netdev_err(dev->net, "write GPIO failed: %d", ret);
				goto error_out;
			}
	...
	if ((ret = ax8817x_write_cmd(dev,
		...
		netdev_err(dev->net, "Select PHY failed: %d", ret);
		goto error_out;
	}

	if ((ret = ax8817x_write_cmd(dev, AX_CMD_SW_RESET,
			AX_SWRESET_IPPD | AX_SWRESET_PRL, 0, 0, NULL)) < 0) {
		netdev_err(dev->net, "Issue sw reset failed: %d", ret);
		goto error_out;
	}

	if ((ret = ax8817x_write_cmd(dev, AX_CMD_WRITE_RX_CTL,
			0, 0, 0, NULL)) < 0) {
		netdev_err(dev->net, "Issue rx ctrl failed: %d", ret);
		goto error_out;
	}
	...
	if ((ret = ax8817x_write_cmd(dev, AX_CMD_WRITE_RX_CTL,
			...
			0, 0, NULL)) < 0) {
		netdev_err(dev->net, "write RX ctrl reg failed: %d", ret);
		goto error_out;
	}
...
static int ax88772_rx_fixup(struct usbnet *dev, struct sk_buff *skb)
{
	...
	while (skb->len > 0) {
		if ((short)(header & 0x0000ffff) !=
			...
			netdev_err(dev->net, "header length data is error 0x%08x, %d\n",
				header, skb->len);
		}
		...
		if (size > ETH_FRAME_LEN) {
			netdev_err(dev->net, "invalid rx length %d", size);
			return 0;
		}
...
static struct sk_buff *ax88772b_tx_fixup(struct usbnet *dev, struct sk_buff *skb, gfp_t flags)
{
	int padlen = ((skb->len + 4) % 512) ? 0 : 4;
	u32 packet_len;
	u32 padbytes = 0xffff0000;
...
static void ax88772_link_reset (struct work_struct *work)
{
	...
	if (ax772_data->Event == AX_SET_RX_CFG) {
		...
		bmcr = ax8817x_mdio_read(dev->net, 
				dev->mii.phy_id, MII_BMCR);
...
static void ax88772a_link_reset (struct work_struct *work)
{
	...
	case CHK_CABLE_EXIST:
		phy_reg = ax8817x_mdio_read(dev->net, dev->mii.phy_id, 0x12);
...
static void ax88772b_link_reset (struct work_struct *work)
{
	...
	case AX_SET_RX_CFG:
	{
		u16 bmcr = ax8817x_mdio_read(dev->net,
					dev->mii.phy_id, MII_BMCR);
...
static int ax88178_set_media(struct usbnet *dev)
{
	...
	if ((ret = ax8817x_write_cmd(dev, AX_CMD_WRITE_MEDIUM_MODE, 
			media, 0, 0, NULL)) < 0) {
		netdev_err(dev->net, "write mode medium reg failed: %d", ret);
		return ret;
	}
