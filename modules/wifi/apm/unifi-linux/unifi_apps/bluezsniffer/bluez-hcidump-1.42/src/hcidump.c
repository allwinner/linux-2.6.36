...
static int process_frames(int dev, int sock, int fd, unsigned long flags)
{
	...
	while (1) {
        ...
        polltimeout = smeConnection?-1:500;
#endif
        /* ------------------------------------------------------------- */
        (void)fflush(stdout);
...
#ifdef IPC_IP
        if (smeConnection && fds[1].revents & (POLLIN | POLLPRI))
        {
...
{
	printf(
	"Usage: bluezcoexsnifferfat [OPTION...] [filter]\n"
    "  -i, --device=hci_dev       HCI device\n"
    "  -a, --address=address      Device Address\n"
...
int find_device_by_address_callback(int dd, int dev_id, long arg)
{
...
int main(int argc, char *argv[])
{
	...
	while ((opt=getopt_long(argc, argv, "a:i:l:p:m:w:r:s:n:d:taxXRC:H:O:P:D:A:BVYZN46h", main_options, NULL)) != -1) {
		switch(opt) {
		case 'i':
		    if (strncmp("hci", optarg, 3) == 0)
		    {
