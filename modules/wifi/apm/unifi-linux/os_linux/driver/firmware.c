...
#if 1 
    priv->fw_sta.dl_data=STA_IMAGE;
    priv->fw_sta.dl_len=sizeof(STA_IMAGE);
#else
    const struct firmware *fw_entry;
    int postfix;
#define UNIFI_MAX_FW_PATH_LEN      64 
    ...
    if (is_fw == UNIFI_FW_STA) {
        ...
        scnprintf(fw_name, UNIFI_MAX_FW_PATH_LEN, "/lib/firmware/%s", "sta.xbv");
	printk("Unifi request fw %s\n", fw_name);
