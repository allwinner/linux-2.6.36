...
dhd_module_init(void)
{
	...
	do {
		...
		if ((dhd_watchdog_prio < 0) && (dhd_dpc_prio < 0)) {
			printk(KERN_ERR"fxz-%s: dhd_watchdog_prio < 0) && (dhd_dpc_prio < 0)\n", __func__);
			break;
		}

		/* If both watchdog and DPC are threads, TX must be deferred */
		if ((dhd_watchdog_prio >= 0) && (dhd_dpc_prio >= 0) && dhd_deferred_tx) {
			printk(KERN_ERR"fxz-%s:dhd_watchdog_prio >= 0) && (dhd_dpc_prio >= 0) && dhd_deferred_tx\n", __func__);
			break;
		}
	...
	error = wifi_add_dev();
	if (error) {
		printk(KERN_ERR"fxz-%s:  wifi_add_dev error\n", __func__);
	...
	if (down_timeout(&dhd_registration_sem,  msecs_to_jiffies(DHD_REGISTRATION_TIMEOUT)) != 0) {
		printk(KERN_ERR"fxz-%s:  down timeout\n", __func__);
		error = -EINVAL;
