...
dhdsdio_membytes(dhd_bus_t *bus, bool write, uint32 address, uint8 *data, uint size)
{
	...
	if ((sdaddr + size) & SBSDIO_SBWINDOW_MASK)
		...
		dsize = size;
	printk(KERN_ERR"fxz-%s: write[%d], address[%08x], size[%d], dsize[%d]\n", __func__, write, address, size, dsize);
	...
	/* Do the transfer(s) */
	while (size) {
		DHD_ERROR(("%s: %s %d bytes at offset 0x%08x in window 0x%08x\n",
...
dhdsdio_probe_init(dhd_bus_t *bus, osl_t *osh, void *sdh)
{
	...
	if (bcmsdh_iovar_op(sdh, "sd_divisor", NULL, 0,
		...
		bus->sd_divisor = -1;
		printk(KERN_ERR"fxz-%s: sd_divisor failed\n", __func__);
	} else {
		DHD_INFO(("%s: Initial value for %s is %d\n",
		          __FUNCTION__, "sd_divisor", bus->sd_divisor));
		          printk(KERN_ERR"fxz-%s: sd_divisor[%d]\n", __func__, bus->sd_divisor);
	}
	...
	if (bcmsdh_iovar_op(sdh, "sd_mode", NULL, 0,
		...
		bus->sd_mode = -1;
		printk(KERN_ERR"fxz-%s: sd_mode failed\n", __func__);
	} else {
		DHD_INFO(("%s: Initial value for %s is %d\n",
		          __FUNCTION__, "sd_mode", bus->sd_mode));
		          printk(KERN_ERR"fxz-%s: sd_mode[%d]\n", __func__, bus->sd_mode);
	}
	...
	if (bcmsdh_iovar_op(sdh, "sd_blocksize", &fnum, sizeof(int32),
		...
		bus->blocksize = 0;
		DHD_ERROR(("%s: fail on %s get\n", __FUNCTION__, "sd_blocksize"));
		printk(KERN_ERR"fxz-%s: sd_blocksize failed\n", __func__);
	} else {
		DHD_INFO(("%s: Initial value for %s is %d\n",
		          __FUNCTION__, "sd_blocksize", bus->blocksize));
		          printk(KERN_ERR"fxz-%s: sd_blocksize[%d]\n", __func__,  bus->blocksize);
	}
	bus->roundup = MIN(max_roundup, bus->blocksize);
	...
	if (bcmsdh_iovar_op(sdh, "sd_rxchain", NULL, 0,
	                    ...
	                    printk(KERN_ERR"fxz-%s: sd_rxchain failed\n", __func__);
		bus->sd_rxchain = FALSE;
	} else {
		DHD_INFO(("%s: bus module (through bcmsdh API) %s chaining\n",
		          __FUNCTION__, (bus->sd_rxchain ? "supports" : "does not support")));
		          printk(KERN_ERR"fxz-%s: sd_rxchain[%d]\n", __func__, bus->sd_rxchain);
	}
                          ...
                          char *fw_path, char *nv_path)
{
    ...
    printk(">>> dhd: download firmware %s, %s\n", fw_path, nv_path);
...
dhdsdio_download_code_file(struct dhd_bus *bus, char *fw_path)
{
	...
	ptr = strstr(fw_path, ".bin");
	if (NULL != ptr) {
		*(ptr + 4) = '\0';
	}
	printk(KERN_ERR"fxz-%s: path[%s]\n", __func__, fw_path);
