                              ...
                              mlme_bss_description_t *new_bss)
{
   ...
   if (!wei_equal_bssid(net->bssId_AP, new_bss->bssId)
       || !wei_equal_ssid(net->bss_p->bss_description_p->ie.ssid, new_bss->bss_description_p->ie.ssid))
      return FALSE;
...
wei_netlist_add_net_to_active_list(mlme_bss_description_t *new_bss)
{
   ...
   for(net = netlist->active_list_ref; 
       net != NULL;
       net = WEI_GET_NEXT_LIST_ENTRY_NAMED(net, WiFiEngine_net_t, active_list)) {
      if(wei_update_net_from_bss_descr(net, new_bss)) {
         /* very chatty */
         //DE_TRACE_STATIC(TR_NETLIST, "Found net in active_list\n");        
         return net;
      }
   }
   ...
   DE_MEMCPY(net->peer_mac.octet, new_bss->bssId.octet, sizeof(new_bss->bssId.octet));
   DE_MEMCPY(net->bssId_AP.octet, new_bss->bssId.octet, sizeof new_bss->bssId.octet);
   WrapperCopy_mlme_bss_description_t(net->bss_p, net->bss_p, new_bss);
