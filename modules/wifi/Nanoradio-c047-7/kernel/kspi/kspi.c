/*
 * $Id: kspi.c 10396 2008-11-17 09:45:58Z peva $
 */

/*! @defgroup kspi_generic Generic SPI implementation on top of kernel driver
 *
 * @brief Generic SPI implementation on top of kernel driver
 *
...
struct spi_dev_t {
	 ...
	 */
	/* It can be removed if CMD52 can be issued between successive CMD53
	 * of the same packet.
	 */
	 ...
	 */
	/* Can be removed if nano_spi_send_cmd52(), nano_spi_read_cmd53()
	 * and nano_spi_send_cmd53() use synchronous calls to kernel SPI API.
	 */
...
static void spi_send(struct sk_buff *skb)
{
	...
	while(remaining_len) {
		...
		if(remaining_len > MAX_CMD53_PER_MSG * MAX_TRANSFER_SIZE) 
			transfer_len = MAX_CMD53_PER_MSG * MAX_TRANSFER_SIZE;
		else 
...
static int spi_send_single(void* buffer, uint32_t len)
{
	...
	status = nano_spi_send_cmd53((unsigned char *) buffer, len);
	TRSP_ASSERT(status == 0);
...
static void spi_recv(nrx_work_data_t *dummy)
{
	...
	if (noise_packet) {
		...
		/* Enable interrupt from target again and exit */
		up(&spi_dev.power_ctl_sem);
...
static void spi_recv_single(void* buffer, unsigned int len)
{
	...
	status = nano_spi_read_cmd53((unsigned char *) buffer, len);
	TRSP_ASSERT(status == 0);
 ...
 *
 * 1. Register to Linux kernel SPI API.\n
 * 2. Setup GPIO pins on the host.\n
 * 3. Setup internal synchronization primitives such as workqueues and
 *    semaphores.\n
 ...
 *    layers.\n
 * 6. Register /proc-files which will capture statistics of the SPI driver.
...
int __init spi_init(void)
{
	...
	/* register to the linux spi api */
	status = nano_spi_register();
	if (status) goto exit;
	...
	printk("nano_kspi: *** Debug pins active ***\n");
#endif
...
exit:
	if (status) {
		printk("nano_kspi: *** init failed, init_level = %d\n", init_level);
		switch (init_level) {
			case 5:
			case 4:
				spi_interrupt(IRQ_OP_DISABLE);
...
static void spi_reset_target(void)
{
   ...
   mdelay (250);
	 ...
	 */
	status = nano_spi_send_cmd52(XMAC_RESET, 0);
	TRSP_ASSERT(status == 0);
...
static void do_sleep(nrx_work_data_t *data)
{
	...
	statistics.sleep++;
	status = nano_spi_send_cmd52(XMAC_ENABLE_SLEEP, 0);
	TRSP_ASSERT(status == 0);
...
static void do_wakeup(nrx_work_data_t *data)
{
	 ...
	 */
	statistics.wakeup++;
	status = nano_spi_send_cmd52(XMAC_DISABLE_SLEEP, 1);
	TRSP_ASSERT(status == 0);
