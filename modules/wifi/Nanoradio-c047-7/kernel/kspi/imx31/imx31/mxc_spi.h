...
typedef struct {
volatile unsigned long CSPIRXDATA	;
volatile unsigned long CSPITXDATA	;
volatile unsigned long CSPICTRL		;
volatile unsigned long CSPIINT		;
volatile unsigned long CSPIDMA		;
volatile unsigned long CSPISTAT		;
volatile unsigned long CSPIPERIOD	;
volatile unsigned long CSPITEST	;
} MXC_CSPI_REG, *PMXC_CSPI_REG;
...
#define imx31_gpio_readl(x)	__raw_readl((IMX31_GPIO_BASE)+(x))
#define imx31_gpio_readw(x)	__raw_readw((IMX31_GPIO_BASE)+(x))
#define imx31_gpio_readb(x)	__raw_readb((IMX31_GPIO_BASE)+(x))

#define imx31_gpio_writel(v,x)	__raw_writel((v),(IMX31_GPIO_BASE)+(x))
#define imx31_gpio_writew(v,x)	__raw_writew((v),(IMX31_GPIO_BASE)+(x))
#define imx31_gpio_writeb(v,x)	__raw_writeb((v),(IMX31_GPIO_BASE)+(x))
