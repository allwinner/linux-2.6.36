 ...
 */
/* $Id: nano_usb.c 16437 2010-10-14 08:44:26Z cjlu $ */
...
nrx_reset(struct nrxdev *dev)
{
   ...
   if (((dev->mask_id >> 8) & 0xFF) == 0x20) {
      ...
      /* NRX701: Reset bits in CCCR_RESET are active low. */
      KDEBUG(TRANSPORT, "Reset NRX701");
...
nano_usb_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
   ...
   if (dev->vendor == 0x03bb) {
      switch (dev->mask_id & 0xFF00) {
         case 0x0000:
            dev_info(&dev->udev->dev, "Nanoradio NRX701\n");
            break;
         case 0x2000:
