/* $Id: nanoutil.h 16507 2010-10-25 10:51:35Z phth $ */
#ifndef _NANOUTIL_H
#define _NANOUTIL_H
...
#  define SYSCTLENTRY(N, D, H) .procname = #N, .data = &D, .maxlen = sizeof(D), .mode = 0600, .proc_handler = H
#  define SYSCTLEND .procname = NULL
...
#  endif
#  define SYSCTLENTRY(N, D, H) .ctl_name = CTL_ANY, .procname = #N, .data = &D, .maxlen = sizeof(D), .mode = 0600, .proc_handler = H
#  define SYSCTLEND .ctl_name = CTL_NONE, .procname = NULL
