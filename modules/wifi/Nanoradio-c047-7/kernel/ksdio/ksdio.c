 ...
 * Supported values are 1 for NRX701 and 2 for NRX600.
 *
...
#define DEVICE_ID_NRX701  0x0000
...
nrx_reset(struct sdio_func *func) 
{
...
#else /* !defined(KSDIO_HOST_RESET_PIN) */
   {
       ...
       * and they are active low for NRX701
       */
      switch (nrxdev->chip_type) {
         case CHIP_TYPE_NRX701:
...
nano_download(const void *buf, size_t size, void *_nrxdev)
{
    ...
    */
   prev_clk = mmc_set_clock(func->card->host, 16000000);
   while (remain > 0) {
      size_t chunk_len = min(remain, (unsigned) KSDIO_FW_CHUNK_LEN);
      memcpy(load_data, data, chunk_len);
      err = sdio_writesb(func, 0, load_data, chunk_len);
      if (err) {
         KDEBUG(ERROR, "fw download failed, err = %d "
                "- chunk #%u, %zu bytes, %zu bytes remain",
                err, chunk, chunk_len, remain);
         break;
      }
      remain -= chunk_len;
      data   += chunk_len;
   ...
   if (err == 0) {
      ...
      /* Give fw time to restart. Measured req time for NRX701
   ...
   switch (func->device & DEVICE_ID_MASK) {
      case DEVICE_ID_NRX600:
         *chip_type = CHIP_TYPE_NRX600;
         return 0;
      case DEVICE_ID_NRX701:
         *chip_type = CHIP_TYPE_NRX701;
         return 0;
      default:
         return -EINVAL;
   }
